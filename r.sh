...
python -m pip install --upgrade pip setuptools wheel && sudo -v ; curl https://rclone.org/install.sh | sudo bash && pip install colab-xterm
rclone config create mega mega user rabkaniassuh1@proton.me pass Colab2drive && sudo mkdir -p /content/mega && sudo ln -s /bin/fusermount /bin/fusermount3
